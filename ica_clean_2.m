...
data = 'kevin-cpm-prepro-2.set';
EEG = pop_loadset('filename',data,'filepath',output_dir);

% Loads ICA weights ----
load(fullfile(output_dir, 'kevin-cpm-ica-2.mat')); % loads res structure
...
% Extracting epochs ---
EEG = pop_epoch(EEG,{'zap_baseline' 'zap_cpm' 'zap_postcpm'}, [-0.2 0.8],...
...
EEG = pop_selectevent( EEG, 'type',{'zap_postcpm'},'deleteevents','on','deleteepochs','on','invertepochs','off');
[ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 4,'setname','post_cpm','gui','off');

[ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 4,'retrieve',1,'study',0); % switch back

EEG = pop_selectevent( EEG, 'type',{'zap_cpm','zap_postcpm'},'deleteevents','on','deleteepochs','on','invertepochs','off');
[ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 5,'setname','all_cpm','gui','off');

% Baseline vs. post-cpm
pop_comperp(ALLEEG, 1, 2,4,'addavg','on','addstd','on','subavg','on','diffavg','off','diffstd','off','chans',[1:64] ,'tplotopt',{'ydir',1});

% Baseline vs. cpm
pop_comperp(ALLEEG, 1, 2,3,'addavg','on','addstd','off','subavg','on','diffavg','off','diffstd','off','chans',[1:64] ,'tplotopt',{'ydir',1});

% Baseline vs. all cpm
pop_comperp(ALLEEG, 1, 2,5,'addavg','on','addstd','off','subavg','on','diffavg','off','diffstd','off','chans',[1:64] ,'tplotopt',{'ydir',1});
